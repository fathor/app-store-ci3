    ...
    <div class="container-fluid">
      <div class="panel panel-headline">
        <div class="panel-heading">
          <h3 class="panel-title">Data Master Pihak Ke Tiga</h3>
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label >Nama</label>
                  <input type="text" id="thrid_party_name" name="thrid_party_name" class="form-control" placeholder="Input Nama ...." required>
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
...
function load_table(){
  table = $('#table').DataTable({
    "order": [],
    "ajax": {
          "url": "<?php echo site_url('thrid_party/load_data')?>",
          "type": "POST",  
    },
  });
}
...
$('#form-input').submit(function(e){
  e.preventDefault();
  $.ajax({
    url:'<?php echo base_url();?>thrid_party/add_data',
    type:"post",
...
function getData(id){
  $.ajax({
    url : "<?php echo site_url('thrid_party/getData/')?>"+id,
    type: "POST",
    dataType: "JSON",
    success: function(data) {
      if (data) {
        ...
        $('#id').val(data.id);
        $('#thrid_party_name').val(data.thrid_party_name);
...
function updateData(){
  $.ajax({
    url : "<?php echo site_url('thrid_party/updateData')?>",
    type: "POST",
    data: {
      id:$('#id').val(),
      thrid_party_name:$('#thrid_party_name').val(),
...
function hapus(id){
  ...
  .then(willDelete => {
    if (willDelete) {
      $.ajax({
        url : "<?php echo site_url('thrid_party/delete')?>/"+id,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {

          ...
          <div class="panel-body">
           <div class="row">
            ...
            <div class="box-body">
              <hr>
              <div class="text-right">
                <h3> Saldo Awal : <span id="balanceEarly"></span></h3>
              </div>
              <hr>
              <div class="table-responsive">
            <div class="card-body card-dashboard" id="tableData"></div>
...
<script>
  ...
  $(document).ready(function() {
    $('#filter_account_code').load("<?php echo site_url('ledger/getAccount'); ?>");
  ...
  function load_table(){
    table = $('#table').DataTable({
      "footerCallback": function ( row, data, start, end, display ) {
        ...
        $( api.column( 5 ).footer() ).text(formatNumber(db));
        $( api.column( 6 ).footer() ).text(formatNumber(kd));
      },
      "destroy": "true",
      "order": [],
      "dom" : 'Bfrtip',
      ...
      "ajax": {
        "url": "<?php echo site_url('ledger/load_data')?>",
        "type": "POST",
        "data": function ( data ) {
  ...
  function getBalanceEarly(){
    $.ajax({
      url : "<?php echo site_url('ledger/getBalanceEarly')?>/",
      type: "POST",
      dataType: "JSON",
      data: {
        'first_date' : $('#filter_first_date').val(),
        'last_date' : $('#filter_last_date').val(),
        'filter_account_code' : $('#filter_account_code').val()
      },
      success: function(data){
        $('#balanceEarly').text(data.balanceEarly);
  ...
  function getBalanceEnd(){
    $.ajax({
      url : "<?php echo site_url('ledger/getBalanceEnd')?>/",
      type: "POST",
      dataType: "JSON",
      data: {
        'first_date' : $('#filter_first_date').val(),
      ...
      success: function(data){
        $('#balanceEnd').text(data.balanceEnd);
      },error: function (jqXHR, textStatus, errorThrown){swal('Terjadi Kesalahan GET Data!')}
    });
  }
